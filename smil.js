...
var indices = new Object();   // id -> animator
...
function initSMIL(evt) {
  ...
  if(!svgDocument.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#SVG-animation", "1.1")) {
  
    var types = ["set", "animate", "animateColor", "animateMotion", "animateTransform"];
    for (var i=0; i<types.length; i++) {
      var animates = O.getElementsByTagName(types[i]);
    ...
    // wrong : I should register these in document order
...
Animator.prototype = {
   ...
   * It schedule the beginings and endings
   */
  register : function() {
  ...
  start : function() {
		...
		if (this.by && this.currFrom)
			this.currTo = add(normalize(this.currFrom), normalize(this.by));
		else
			this.currTo = this.to;
  },
   ...
   */
	 ...
	 */
	getPath : function() {
	  var mpath = this.anim.getElementsByTagName("mpath")[0];
	  if (mpath) {
	    ...
	    if (path) {
				var pathEl = svgDocument.createElementNS(svgns, 'path');
...
// vérifier les spec pour le coup du end finish et repeat. faut-il pas tt simplement fusionner end et finish
// implication au niveau des id.sync-base et des fill="remove"

// vérifier si les <set> sont bien optimisés